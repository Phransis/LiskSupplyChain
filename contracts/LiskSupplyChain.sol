...
contract LiskSupplyChain {
    ...
    mapping(uint256 => Status) public prodStatus;
    ...
    modifier isProductIdValid(uint256 productId) {
        require(
    ...
    function updateProductStatus(uint256 productId, Status status)
        public
        onlyOwner(productId)
    ...
    function changeOwnership(uint256 productId, address newOwner)
        public
        onlyOwner(productId)
        isProductIdValid(productId)
    {
    ...
    function viewStatus(uint256 productId)
        public
        view
        isProductIdValid(productId)
        returns (string memory)
    {
        Status _status = prodStatus[productId];
        return checkStatus(_status);
    ...
    //Function to check a product's status
    function productStatus(uint256 productId)
        public
        view
        isProductIdValid(productId)
        returns (string memory)
    {
        ...
        if (products[productId].status == Status.Created) {
            return "Created";
        } else if (products[productId].status == Status.Sold) {
            return "Sold";
        } else if (products[productId].status == Status.Shipped) {
            return "Shipped";
        } else if (products[productId].status == Status.Received) {
            return "Received";
        } else {
            return "Unknown";
        }
